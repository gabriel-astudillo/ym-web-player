/*
 * aym-playlist.js - Copyright (c) 2001-2026 - Olivier Poncet
 *
...
export class AYM_Playlist {
    ...
    const target = index | 0;
        if (target >= this.min_track && target <= this.max_track) {
            this.cur_track = target;
            return this.getMusic();
        }
        return null;
    }
    ...
    getTrackNames() {
        // Retorna una lista con el campo .name de cada objeto de música en Playlist
        //return Playlist.map(music => music.title + " - " + music.comments || "Unknown");
        return Playlist.map(music => music.title || "Unknown");
    }
